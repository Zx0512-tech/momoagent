"""临时脚本：全量重建 SUBMISSION_MANIFEST.json。用后即删。
...
verify_submission.py 只校验 fileCount 与 files 条目数自洽，不做文件系统比对，
...
RESCAN_DIRS = (
    "analysis_data",
    # 取整个 docs/examples 而不只是 templates：real_agent_baseline_manifest.json
...
# 防呆
assert not any(f["path"] == "SUBMISSION_MANIFEST.json" for f in files)
