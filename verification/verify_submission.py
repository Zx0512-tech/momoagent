...
BACKEND = REPOSITORY_ROOT / "momo_agent" / "backend"
RUNTIME_PARENT = REPOSITORY_ROOT / "bridge_models" / "stbridge_opensees"
EVIDENCE = VERIFICATION_ROOT / "evidence" / "opensees_optimization_20260802"
MANIFEST = VERIFICATION_ROOT / "SUBMISSION_MANIFEST.json"


def require(path: Path) -> None:
    if not path.exists():
        raise FileNotFoundError(f"缺少提交文件：{path.relative_to(REPOSITORY_ROOT)}")
...
def main() -> None:
    ...
    required = [
        BACKEND / "app" / "main.py",
        REPOSITORY_ROOT / "platform-ui" / "dist" / "index.html",
        REPOSITORY_ROOT / "pyansys_bridge" / "optimization" / "config_runner.py",
        REPOSITORY_ROOT / "bridge_models" / "stbridge_opensees" / "stbridge_opensees_modal_builder.py",
        REPOSITORY_ROOT / "analysis_data" / "earthquake_inputs" / "earthquake_acceleration_record.txt",
        REPOSITORY_ROOT / "analysis_data" / "wind_inputs" / "wind_vertical_8nodes_10mps_3600s.csv",
        REPOSITORY_ROOT / "docs" / "examples" / "templates" / "openseespy_inproc_run_joint_baseline_workflow_template.json",
    ...
    manifest = verify_submission_manifest(REPOSITORY_ROOT, MANIFEST)

    sys.path[:0] = [str(REPOSITORY_ROOT), str(BACKEND), str(RUNTIME_PARENT)]
