...
class PlatformStore:
    ...
    def record_worker_heartbeat(self, job_id: str, *, phase: str = '执行中') -> Job:
        ...
        job.progress = JobProgress(phase=phase, message='worker 心跳正常', percent=job.progress.percent if job.progress else None)
    ...
    def record_case_progress(self, job_id: str) -> Job:
        ...
        # 进度文件可能先写完批次计数，随后才由执行器落库终态；RUNNING 期间
        # 即使没有可识别的活动算例，也不能把任务提前标成 100%。
...
platform_store = PlatformStore(recover_orphans=os.environ.get('MOMO_PLATFORM_WORKER') != '1')
