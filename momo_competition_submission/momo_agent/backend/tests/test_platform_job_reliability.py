    ...
    completed = subprocess.run(
        [
            ...
            (
                ...
                f'print(platform_store.get_job({job.job_id!r}).status)'
            ),
        ],
...
def test_progress_collection_failure_does_not_terminate_worker(
    ...
    def broken_progress(_self, _job_id: str):
        raise ValueError('broken progress payload')

    monkeypatch.setattr(PlatformStore, 'record_case_progress', broken_progress)
