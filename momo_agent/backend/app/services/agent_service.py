...
DEFAULT_SESSION_TITLE = 'MOMO 工程智能体'
...
class AgentService(WorkflowHarnessMixin, AgentConversationMixin):
    ...
    def create_message(
        ...
        *,
        ...
        now = utc_now()
        # 前端创建会话时只有固定占位标题；首条消息到达后才有可总结内容。
        should_summarize_title = (
            str(session.get('title') or '') == DEFAULT_SESSION_TITLE
            and not repository.has_messages(session['sessionId'])
        )
        ...
        # 工程消息完成后再启动标题请求，避免与主流程争用同一个模型连接。
        if should_summarize_title:
            self._schedule_session_title(session['sessionId'], content)
        return run

    def _schedule_session_title(self, session_id: str, first_message: str) -> None:
        """异步生成会话标题；展示性失败不影响工程消息主流程。"""
        def _run() -> None:
            try:
                title = self.planner.summarize_session_title(first_message)
            except LLMUnavailableError as exc:
                logger.info('会话 %s 标题生成失败，保留原标题：%s', session_id, exc.reason)
                return
            except Exception:
                logger.warning('会话 %s 标题生成异常，保留原标题', session_id, exc_info=True)
                return
            try:
                self.repository().update_session_title(session_id, title)
            except Exception:
                logger.warning('会话 %s 标题落库失败', session_id, exc_info=True)

        threading.Thread(
            target=_run,
            name=f'agent-session-title-{session_id}',
            daemon=True,
        ).start()
