...
def test_worker_observation_persists_heartbeat_and_progress_in_one_update(tmp_path: Path) -> None:
    """同一轮观测只能落库一次，API 不能读到心跳与案例进度之间的半帧。"""

    store = PlatformStore(state_path=tmp_path / 'state.sqlite3')
    job = running_job(store)
    write_batch_progress(job_progress_dir(job.job_id), completed=8, total=15)
    update_calls = 0
    original_update = store.repository.update_job
    ...
    store.repository.update_job = counting_update

    updated = store.record_worker_observation(job.job_id, phase='求解中')

    assert update_calls == 1
    assert updated.worker is not None
    assert updated.worker.heartbeat_at is not None
    assert updated.progress.completed_cases == 8
    assert updated.progress.total_cases == 15


def test_heartbeat_preserves_existing_case_progress(tmp_path: Path) -> None:
    """只有心跳的新帧不得清空上一份已持久化的批量与活动算例。"""

    state_path = tmp_path / 'state.sqlite3'
    store = PlatformStore(state_path=state_path)
    job = running_job(store)
    directory = job_progress_dir(job.job_id)
    write_batch_progress(directory, completed=8, total=15)
    write_case_progress(directory, 'case_09', step=42, total_steps=100)
    ...
    assert persisted.progress.completed_cases == 8
    assert persisted.progress.total_cases == 15
    assert [case.case_id for case in persisted.progress.active_cases] == ['case_09']
